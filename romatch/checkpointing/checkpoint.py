...
class CheckPoint2:
    def __init__(self, dir=None, name="tmp", model_name = "diffusion"):
        self.name = name
    ...
    def save(
        ...
        if romatch.RANK == 0:
            ...
            torch.save(states, self.dir + self.name + f"_{self.model_name}_latest.pth")
            logger.info(f"Saved states {list(states.keys())}, at step {n}")
    ...
    def load(
        ...
        if os.path.exists(self.dir + self.name + f"_{self.model_name}_latest.pth") and romatch.RANK == 0:
            states = torch.load(self.dir + self.name + f"_{self.model_name}_latest.pth")
            if "model" in states:
                model.load_state_dict(states["model"])
            if "n" in states:
                n = states["n"] if states["n"] else n
