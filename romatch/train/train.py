...
def train_step_new(train_batch, model, objective, optimizer, grad_scaler, grad_clip_norm = 1.,**kwargs):
    optimizer.zero_grad()
    out, mask_pyramid, pre_theta, pre_u, pre_v = model(train_batch, mode = 'BEV', end2end=True) #corresps, mask_pyramid, pre_theta, pre_u, pre_v
    l = objective(out, train_batch, mask_pyramid, pre_theta, pre_u, pre_v)
    grad_scaler.scale(l).backward()
    grad_scaler.unscale_(optimizer)
    log_param_statistics(model.named_parameters())
    torch.nn.utils.clip_grad_norm_(model.parameters(), grad_clip_norm) # what should max norm be?
...
def train_k_steps_new(
    n_0, k, dataloader, dataloader_iter, model, objective, optimizer, lr_scheduler, grad_scaler, progress_bar=True, grad_clip_norm = 1., warmup = None, ema_model = None, pbar_n_seconds = 1,
):
    for n in tqdm(range(n_0, n_0 + k), disable=(not progress_bar) or romatch.RANK > 0, mininterval=pbar_n_seconds):
        try:
            batch = next(dataloader_iter)  # Get the next batch
