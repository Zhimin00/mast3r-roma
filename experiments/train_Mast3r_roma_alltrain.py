...
def get_SD_model(pretrained_backbone=True, resolution = "medium", **kwargs):
    ...
    encoder = CNNandMast3r_trainable(
        cnn_kwargs = dict(
    ...
    matcher = RegressionMatcher_mast3r(encoder, decoder, h=h, w=w,**kwargs)
    ...
    # weights = torch.load('/cis/home/zshao14/Downloads/RoMa_mast3r/experiments/workspace/mast3r/train_Mast3r_roma_latest2.pth', map_location='cuda')['model']
    # matcher.load_state_dict(weights, strict=False)
    return matcher
...
def train(args):
    ...
    print('Mast3r-RoMa parameters:', model_params)
    ...
    dataset_path = '/cis/net/io104/data/zshao14/megadepth'
    # Data
    ...
    depth_loss = RobustLosses(
        ...
        c = 1e-4,)
    parameters = [
        {"params": model.encoder.base_model.parameters(), "lr": romatch.STEP_SIZE * 1e-4 / 8},
        {"params": model.encoder.cnn.parameters(), "lr": romatch.STEP_SIZE * 1e-4 / 8},
    ...
    megadense_benchmark = MegadepthDenseBenchmark(dataset_path, num_samples = 1000, h=h,w=w)
