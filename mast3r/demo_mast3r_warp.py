...
if __name__ == '__main__':
    ...
    model = AsymmetricMASt3R_only_warp.from_pretrained(model_name).to(device)
    ...
    output_path = '/cis/net/r24a/data/zshao/data/wriva_processed_data/cross-view/vis_all/A01_s02_2'
    images = load_images([im1_path, im2_path], size=512)
    batch = [tuple(images)]
    batch = collate_with_cat(batch[:1])
    view1, view2 = make_batch_symmetric(batch)
    _, _, corresps = model(view1, view2)
    warp, dense_certainty = dense_match(corresps)
    ...
    canvas[:H, :W] = im1.permute(1,2,0)
    canvas[:H, W:] = im2.permute(1,2,0)
    offset = np.array([W, 0])
