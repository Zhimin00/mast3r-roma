...
def get_patch_embed(patch_embed_cls, img_size, patch_size, enc_embed_dim, **kwargs):
    assert patch_embed_cls in ['PatchEmbedDust3R', 'ManyAR_PatchEmbed','PatchEmbedDust3R2', 'ManyAR_PatchEmbed2', 'ManyAR_PatchEmbed_ResNet', 'ManyAR_PatchEmbed_cnn']
...
class ManyAR_PatchEmbed_cnn (PatchEmbed):
    ...
    def forward(self, img, true_shape):
        ...
        x = img.new_zeros((B, n_tokens, self.embed_dim))
        pos = img.new_zeros((B, n_tokens, 2), dtype=torch.int64)
        feats = [img.new_zeros((B, ns[idx], self.cnn_features_dims[idx])) for idx in range(len(self.cnn_feature_dims))]
