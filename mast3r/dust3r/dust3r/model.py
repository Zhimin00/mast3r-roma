...
class AsymmetricCroCo3DStereo_cnn (
    ...
    def set_freeze(self, freeze):  # this is for use by downstream models
        ...
        to_be_frozen = {
            ...
            'mask': [self.mask_token],
            'encoder': [self.mask_token, self.patch_embed, self.enc_blocks],
            'backbone': [self.mask_token, self.enc_norm, self.decoder_embed, self.dec_norm, self.patch_embed, self.enc_blocks, self.dec_blocks, self.dec_blocks2, self.downstream_head1, self.downstream_head2]
        }
        freeze_all_params(to_be_frozen[freeze])
