...
from dust3r.inference import loss_of_one_batch, loss_of_one_batch_warp, loss_of_one_batch_only_warp  # noqa
...
def train(args):
    ...
    param_groups = misc.get_parameter_groups(model_without_ddp, args.lr, args.weight_decay)
...
def train_one_epoch_only_warp(model: torch.nn.Module, warp_criterion: torch.nn.Module,
                    data_loader: Sized, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, loss_scaler,
                    args,
                    log_writer=None):
    ...
    for data_iter_step, batch in enumerate(metric_logger.log_every(data_loader, args.print_freq, header)):
        ...
        loss_tuple = loss_of_one_batch_only_warp(batch, model, warp_criterion, device,
                                       symmetrize_batch=True,
                                       use_amp=bool(args.amp), ret='loss')
        loss, loss_details = loss_tuple  # criterion returns two values
        loss_value = float(loss)
...
if __name__ == '__main__':
    train_dataset = "57_000 @ Habitat(800_000, split='train', ROOT='/cis/home/cpeng/dust3r/data/habitat_processed', resolution=[(512, 384), (512, 336), (512, 288), (512, 256), (512, 160)], aug_crop='auto', transform=ColorJitter)"
    data_loader_train = build_dataset(train_dataset, 8,8, test=False)
    pdb.set_trace()
