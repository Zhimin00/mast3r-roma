...
def transpose_to_landscape(head, activate=True):
    ...
    """
    def wrapper_no(decout, true_shape, mode='default'):
        B = len(true_shape)
        assert true_shape[0:1].allclose(true_shape), 'true_shape must be all identical'
        H, W = true_shape[0].cpu().tolist()
        res = head(decout, (H, W), mode=mode)
        return res

    def wrapper_yes(decout, true_shape, mode='default'):
        B = len(true_shape)
        ...
        if is_portrait.all():
            return transposed(head(decout, (W, H), mode = mode))

        # batch is a mix of both portraint & landscape
        def selout(ar): return [d[ar] for d in decout]
        l_result = head(selout(is_landscape), (H, W), mode=mode)
        p_result = transposed(head(selout(is_portrait), (W, H), mode = mode))
...
def transpose_to_landscape_cnn(head, activate=True):
    """ Predict in the correct aspect-ratio,
        then transpose the result in landscape 
        and stack everything back together.
    """
    def wrapper_no(decout, cnn_feats, true_shape):
        B = len(true_shape)
        assert true_shape[0:1].allclose(true_shape), 'true_shape must be all identical'
        H, W = true_shape[0].cpu().tolist()
        res = head(decout, cnn_feats, (H, W))
        return res

    def wrapper_yes(decout, cnn_feats, true_shape):
        B = len(true_shape)
        # by definition, the batch is in landscape mode so W >= H
        H, W = int(true_shape.min()), int(true_shape.max())
        ...
        # true_shape = true_shape.cpu()
        if is_landscape.all():
            return head(decout, cnn_feats, (H, W))
        if is_portrait.all():
            return transposed(head(decout, cnn_feats, (W, H)))

        # batch is a mix of both portraint & landscape
        def selout(ar): return [d[ar] for d in decout]
        def cnnout(ar): return [cnn_feat[ar] for cnn_feat in cnn_feats]
        l_result = head(selout(is_landscape), cnnout(is_landscape), (H, W))
        p_result = transposed(head(selout(is_portrait), cnnout(is_portrait), (W, H)))
