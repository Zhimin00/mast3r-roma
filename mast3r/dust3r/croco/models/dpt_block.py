...
class FeatureFusionBlock_custom2(nn.Module):
    ...
    def __init__(
        ...
        self.resConfUnit1 = ResidualConvUnit_custom(features, activation, bn)
        self.resConfUnit2 = ResidualConvUnit_custom(features, activation, bn)
        self.resConfUnit3 = ResidualConvUnit_custom(features, activation, bn)
    ...
    def forward(self, *xs):
        ...
        if len(xs) == 3:
            ...
            if self.width_ratio != 1:
                res = F.interpolate(res, size=(output.shape[2], output.shape[3]), mode='bilinear')
            output = self.skip_add2.add(output, res)
...
def make_fusion_block2(features, use_bn, width_ratio=1):
...
class FeatureFusionBlock_custom3(nn.Module):
    """Feature fusion block."""
    ...
    def __init__(
        ...
        Args:
            features (int): number of features
        """
        super(FeatureFusionBlock_custom3, self).__init__()
        self.width_ratio = width_ratio
    ...
    def forward(self, *xs):
        ...
        if self.width_ratio != 1:
            # and output.shape[3] < self.width_ratio * output.shape[2]
            #size=(image.shape[])
            if (output.shape[3] / output.shape[2]) < (1 / 3) * self.width_ratio:
                shape = 3 * output.shape[3]
            else:
                shape = int(self.width_ratio * output.shape[2])
            output  = F.interpolate(output, size=(output.shape[2], shape), mode='bilinear')
        output = self.out_conv(output)
        return output

def make_fusion_block3(features, use_bn, width_ratio=1):
    return FeatureFusionBlock_custom3(
        features,
        nn.ReLU(False),
        deconv=False,
        bn=use_bn,
...
class DPTOutputAdapter2(nn.Module):
    """DPT output adapter.
    ...
    def __init__(self,
        ...
        self.scratch.refinenet1 = make_fusion_block2(feature_dim, use_bn, output_width_ratio)
    ...
    def forward(self, encoder_tokens: List[torch.Tensor], vgg_features: List[torch.Tensor], image_size):
        assert self.dim_tokens_enc is not None, 'Need to call init(dim_tokens_enc) function first'
