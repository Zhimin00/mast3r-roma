...
from romatch.losses.robust_loss import RobustLosses, RobustLosses_3D
from romatch.benchmarks import MegaDepthPoseEstimationBenchmark, MegadepthDenseBenchmark, HpatchesHomogBenchmark

from romatch.train.train import train_k_steps, train_k_steps_new
...
def train(args):
    ...
    for n in range(romatch.GLOBAL_STEP, N, k * romatch.STEP_SIZE):
        mega_dataloader = iter(data_loader_train)
        train_k_steps_new(
            n, k, data_loader_train, mega_dataloader, ddp_model, depth_loss, optimizer, lr_scheduler, grad_scaler, grad_clip_norm = grad_clip_norm,
        )
